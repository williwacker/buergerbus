...
class Telefonbuch():
    def __init__(self):
        ...
        self.transTable = {'zip': 'pc', 'na': 'na', 'ci': 'ci', 'st': 'st',
                           'hn': 'hn', 'ph': 'ph', 'mph': 'mph', 'recuid': 'recuid'}

    def dasoertliche(self, name, city, typ):
        lurl = self.http.request(
            'GET', 'https://www.dasoertliche.de/?kgs=07331020&choose=true&page=0&context=0&action=43&buc=22&topKw=0&form_name=search_nat&kw={}&ci={}'.format(name, city))
        ...
        try:
            ...
            for m in range(len(handlerData)):
                ...
                if data_dict['pc'] and re.match(
                        "[\d]5", data_dict['pc']) and data_dict['ci'] and data_dict['st'] and data_dict['hn']:
    ...
    def dasoertliche_gewerbe(self, line, data_dict):
        handlerData = re.search(
            'recuid=' + data_dict['recuid'] + '.*?<address>(.*?)\s+([0-9]+.*?),.*?([\s0-9]+).*?<span.*?>(.*?)</span>',
            line).group(
            1, 2, 3, 4)
