...
class FahrerAddView(MyCreateView):
    ...
    def form_valid(self, form):
        ...
        instance.save()
        self.success_message = self.model._meta.verbose_name_raw+' "<a href="'+self.success_url+str(instance.id)+'/'+url_args(
            self.request)+'">'+str(instance)+' '+str(instance.team)+'</a>" wurde erfolgreich hinzugefügt.'
...
class FahrerChangeView(MyUpdateView):
    ...
    def form_valid(self, form):
        ...
        self.success_message = self.model._meta.verbose_name_raw+' "<a href="'+self.success_url+str(
            instance.id)+'">'+str(instance)+' '+str(instance.team)+'</a>" wurde erfolgreich geändert.'
        ...
        if 'team' in form.changed_data:
            ...
            if instance.aktiv:
                ...
                if group:
                    instance.benutzer.groups.add(group)
            self.success_message = self.model._meta.verbose_name_raw+' "<a href="'+self.success_url+str(
                instance.id)+'">'+str(instance)+' '+str(instance.team)+'</a>" wurde erfolgreich hinzugefügt.'
