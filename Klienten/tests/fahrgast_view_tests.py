...
class FahrgastListViewTests(TestCase):
    model = Klienten
    list_url = '/Klienten/fahrgaeste/'
    title = 'Fahrgast'
    title_plural = 'Fahrgäste'

    def setUp(self):
        UserTestCase().test_group()
        UserTestCase().test_user()
        FahrgastTestCase().setUp()
        FahrgastTestCase().test_fahrgast_non_outside_success()

        self.client = Client()
        self.user = User.objects.get(username='testuser')
        self.client.login(username='testuser', password='12345')
        self.group = Group.objects.get(name='Büro ABC')
        id = str(self.model.objects.filter(typ='F').first().id)
