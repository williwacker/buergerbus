...
from Basis.views import (MyCreateView, MyDeleteView, MyListView,
...
class DienstleisterView(MyListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['filter'] = DienstleisterFilter(
            self.request.GET.copy(), queryset=Klienten.objects.order_by('name', 'ort').filter(typ='D'))
        return context
...
class DienstleisterAddView(MyCreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            instance.save()
            messages.success(request, 'Dienstleister "<a href="' + self.success_url + str(instance.id) + '/' +
                             url_args(request) + '">' + instance.name + '</a>" wurde erfolgreich hinzugefügt.')
            context['messages'] = messages
            if self.request.GET.get('_popup') == '1':
                return HttpResponse(
                    '''<script type="text/javascript">opener.location.reload(false);window.close();</script>''')
...
class DienstleisterChangeView(MyUpdateView):
    ...
    def form_valid(self, form):
        ...
        self.success_message = 'Dienstleister "<a href="'+self.success_url+str(
            instance.id)+'">'+instance.name+'</a>" wurde erfolgreich geändert.'
...
class DienstleisterSearchMultiformsView(MyMultiFormsView):
    ...
    def anlegen_form_valid(self, form):
        ...
        self.create_dienstleister(result, force_create=form.cleaned_data.get(
            'force_create', False), city_create=form.cleaned_data.get('city_create', False))
        return HttpResponseRedirect(self.this_url+url_args(self.request))
    ...
    def create_ort(self, plz, ort):
        ...
        ort.save()
        messages.success(
            self.request, 'Ort "<a href="/Klienten/orte/' + str(ort.id) + '/' + '">' + str(ort) +
    ...
    def create_strasse(self, strasse, ort):
        ...
        strasse.save()
        messages.success(self.request, 'Strasse "<a href="/Klienten/strassen/' + str(strasse.id) + '/' + '">' +
                         str(strasse) + '</a>" wurde erfolgreich hinzugefügt.')
        return
    ...
    def create_dienstleister(self, result, force_create=False, city_create=False):
        ...
        if not cities_by_zip:
            if self.request.user.has_perm('Klienten.add_orte'):
                if city_create:
                    ...
                    messages.info(
                        self.request,
                        "Der gefundene Ort existiert noch nicht. Wenn Sie ihn anlegen wollen, bitte 'Ort und Strasse anlegen' anhaken.")
                    self.request.session['city_create'] = True
                    return HttpResponseRedirect(self.success_url+url_args(self.request))
            else:
                messages.info(
                    self.request,
                    "Der gefundene Ort existiert noch nicht, aber Sie haben keine Berechtigung um Orte anzulegen!")
                return HttpResponseRedirect(self.success_url+url_args(self.request))
        best_match = process.extract(
            result['ci'],
            [sub['ort'] for sub in cities_by_zip],
            scorer=fuzz.token_set_ratio, limit=100)  # for debugging only
        filtered_city_names = [
            sub[0]
            for sub in process.extract(
                result['ci'],
                [sub['ort'] for sub in cities_by_zip],
                scorer=fuzz.token_set_ratio) if sub[1] >= 85]
        matching_cities = list(Orte.objects.values('ort', 'id').filter(ort__in=filtered_city_names))

        streets_by_city = list(Strassen.objects.values('strasse', 'ort', 'id').filter(
            ort_id__in=[sub['id'] for sub in matching_cities]))
        ...
        if streets_by_city:
            filtered_street_names = [
                ...
                for sub in process.extract(
                    result['st'],
                    [sub['strasse'] for sub in streets_by_city],
                    scorer=fuzz.token_set_ratio) if sub[1] >= 85]
                ...
                if city_create:
                    ...
                    filtered_street_names = [
                        ...
                        for sub in process.extract(
                            result['st'],
                            [sub['strasse'] for sub in streets_by_city],
                            scorer=fuzz.token_set_ratio, limit=1) if sub[1] >= 85]
                else:
                    messages.info(
                        self.request,
                        "Die gefundene Strasse existiert noch nicht. Wenn Sie sie anlegen wollen, bitte 'Ort und Strasse anlegen' anhaken.")
                    self.request.session['city_create'] = True
                    return HttpResponseRedirect(self.success_url+url_args(self.request))
            else:
                messages.info(
                    self.request,
                    "Die gefundene Strasse existiert noch nicht, aber Sie haben keine Berechtigung um Strassen anzulegen!")
                return HttpResponseRedirect(self.success_url+url_args(self.request))
        matching_street = list(
            Strassen.objects.values('strasse', 'ort', 'id').filter(
                ort_id__in=[sub['id'] for sub in matching_cities],
                strasse__in=filtered_street_names))
...
#		category = dienstleister_category_match[dienstleister_category_match[0].index(matching_category[0])] if matching_category[1] > 85 else ''
        ...
        if filtered_street_names:
            existing_clients = list(Klienten.objects.values('name', 'id').filter(
                ort=matching_street[0]['ort'], strasse=matching_street[0]['id']))
            if existing_clients:
                for client in existing_clients:
                    if process.extractOne(
                            result['na'],
                            client, scorer=fuzz.token_set_ratio)[1] > 95 and not force_create:
                        existing_client = Klienten.objects.get(id=client['id'])
                        messages.info(self.request,
                                      'Ein ähnlich oder gleich lautender Dienstleister existiert bereits: "<a href="' +
                                      self.success_url + str(existing_client.id) + '/">' + existing_client.name + '</a>" ')
        ...
        klient.save()
        messages.success(
            self.request, 'Dienstleister "<a href="' + self.success_url + str(klient.id) + '/">' + klient.name +
            '</a>" wurde erfolgreich hinzugefügt.')
