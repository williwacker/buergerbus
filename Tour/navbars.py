            ...
            for nav in nav_bar:
                ...
                if datum:
                    if int(datum) == nav.id:
                        selected = ' selected'
                        tag = 'span'
                html += '<{4} href="?datum={0}" class="changeform-navigation-button segmented-button {3}"><span class="changeform-navigation-button-label">{1} {2}</span></{4}>'.format(nav.id,nav.datum,nav.team,selected,tag)
        else:
